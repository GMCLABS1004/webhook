...
module.exports=function divide_entry_bitmex(obj){
      ...
      async.waterfall([
        function init(cb){
            var data = {
                ...
                minOrdValue : 10, //최소주문금액
                siteMinValue : 2, //거래소 주문 최소 가치 
                side : obj.side, //주문 타입
        ...
        function orderCancel(data, cb){
            if(data.idx > 1){
                ...
                request(requestHeader, function(error, response, body){
                    ...
                    data.totalRemainVal += (data.canceledPrice * remainAmt); //미체결 가치 합산
                    cb(null, data);
                });
            }else{
                cb(null, data);
        ...
        function calc(data, cb){
            ...
            if(data.side === 'Buy'){
                ...
                data.minOrdValue = fixed4(data.minOrdValue / data.ordPrice); //최소주문가치(xbt) = 수량(달러) / 가격(달러)
                data.siteMinValue = fixed4(data.siteMinValue / data.ordPrice); //최소주문가치(xbt) = 수량(달러) / 가격(달러)
            ...
            if(zziggeogi < data.siteMinValue && zziggeogi > 0){
                //찌꺼기 포함해서 주문
                data.ordAmount = Math.floor(zziggeogi * data.ordPrice); //주문수량
            ...
            if(data.ordValue < data.minOrdValue){
                ...
                //주문최소가치로 주문 
                data.ordAmount = Math.floor(data.minOrdValue * data.ordPrice); //주문수량 다시 계산
        ...
        if(error){
            console.log(error);
            logger.info("분할주문종료")
            switchOnOff({apiKey : obj.apiKey,secreteKey : obj.secreteKey},false);
            return;
        }
        ...
        if(data.isContinue === true){ //분할주문 가능
            data.idx += 1;
            setTimeout(divide_entry_bitmex(data), 10000);//분할주문
            return;
        }
        ...
        // var goalMargin = data.firstMargin - data.goalValue; //목표마진
        // var realMargin = data.firstMargin - data.totalOrdValue; //현재마진
        // if(goalMargin >= realMargin){
        //     //DB저장
