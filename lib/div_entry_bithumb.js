        ...
        function getUserMargin(data, cb){ //잔액조회
            ...
            bithumAPI.bithumPostAPICall('/info/balance', rgParams, function(error, response, body){
                ...
                if(data.idx === 1){
                    ...
                    data.firstMargin = Number(json.data["total_krw"]);
                    data.total_krw = Number(json.data["total_krw"]);
                    data.total_btc = Number(json.data["total_btc"]);
                }
                cb(null, data);
            });
        },
            ...
            bithumAPI.bithumPostAPICall('/info/orders', rgParams, function(error, response, body){
                ...
                if(json.status === "0000"){
                    if(Number(json.data[0].units_remaining) > 0){
                        data.totalOrdAmount -= Number(json.data[0].units_remaining); //주문수량 누적
    ...
    async.waterfall([
        function getUserMargin(cb){ //잔액조회
            ...
            bithumAPI.bithumPostAPICall('/info/balance', rgParams, function(error, response, body){
                ...
                total_btc =  (Number(json.data["total_btc"]));
