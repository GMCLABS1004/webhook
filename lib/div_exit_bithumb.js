      ...
      async.waterfall([
        ...
        function depth(data, cb){
            ...
            bithumAPI.orderbook('BTC',function(error,response, body){
                ...
                data.sellDepth = {}
                 //매수목록 조회
                data.buyDepth = {
                    price : Number(json.data.bids[0].price),//가격
                    amount : Number(json.data.bids[0].quantity),//수량
                    value :  Number(json.data.bids[0].price) * Number(json.data.bids[0].quantity) //가치
                }
                cb(null, data);
            });
        },
        function position(data, cb){ //포지션 조회
            ...
            bithumAPI.bithumPostAPICall('/info/balance', rgParams, function(error, response, body){
                ...
                data.openingQty = Number(json.data["available_btc"]); //현재 이용가능한 코인수량
                ...
                if(data.openingQty >= 0.0001){ //현재이용가능 BTC수량 >= 0.0001
        ...
        function calc(data, cb){ //주문가격, 주문수량, 주문 가치 결정
            ...
            if(data.side === 'none'){ //현재포지션 === NONE 
                ...
                setTimeout(insert_history(bithumAPI, data), 2000); //주문내역 저장
        ...
        function order(data, cb){
            ...
            bithumAPI.bithumPostAPICall('/trade/place', rgParams, function(error, response, body){
                ...
                data.orderID = json.order_id; //주문ID저장
        ...
        function order_info(data, cb){
            ...
            bithumAPI.bithumPostAPICall('/info/orders', rgParams, function(error, response, body){
                ...
                if(json.status === "0000"){
                    ...
                    data.totalOrdValue -= (data.ordPrice *Number(json.data[0].units_remaining));  //총주문가치 = 총주문가치 - (주문가격 * 미체결수량)
                }
                cb(null, data);
            });
        },
        ...
        if(data.isContinue === true){ //분할주문 계속O
            ...
            setTimeout(last_order(bithumAPI, data), 5000); //마지막주문
        }
      });
    }
  }
        ...
        async.waterfall([
            ...
            function position(data, cb){ //분할주문 계속 진행할지 말지 판단
                ...
                bithumAPI.bithumPostAPICall('/info/balance', rgParams, function(error, response, body){
                    ...
                    if(avail_btc < 0.0001){ //분할주문 계속안해도 되면
                        logger.info("")
                        data.isContinue = false; //분할주문 중지
                    }else{
