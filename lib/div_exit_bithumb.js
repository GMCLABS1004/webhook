      ...
      async.waterfall([
        ...
        function depth(data, cb){
            bithumAPI.orderbook('BTC',function(error,response, body){
                ...
                data.sellDepth = {}
                data.buyDepth = {
                    price : Number(json.data.bid[0].price),//가격-> $ dollar
                    amount : Number(json.data.bid[0].quantity),//수량-> $ dollar
                    value :  Number(json.data.bid[0].price) * Number(json.data.bid[0].quantity) //가치 -> xbt
                }
                cb(null, data);
            });
        },
