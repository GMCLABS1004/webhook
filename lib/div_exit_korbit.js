...
// var logger;
// var logfileName1 = '../log/korbit' +'.log'; //로그파일 경로1
// var logfileName2 = '../log/korbit' +'.debug.log'; //로그파일 경로2
...
var logger;
module.exports = divide_exit_korbit;

function divide_exit_korbit(korbit, obj, logObj){
    logger = logObj
      ...
      async.waterfall([
        ...
        function depth(data, cb){
            korbit.orderbook("btc_krw", function(error, response, body){
                ...
                data.buyDepth = {
                    price : Number(json.bids[0][0]),//가격-> $ dollar
                    amount : Number(json.bids[0][1]),//수량-> $ dollar
                    value :  Number(json.bids[0][0]) * Number(json.bids[0][1]) //가치 -> xbt
                }
                data.sellDepth = {}
                cb(null, data);
            });
        ...
        function position(data, cb){
            ...
            korbit.balances(function(error, response, body){
                ...
                data.openingQty = fixed4(Number(json["btc"].available));
        ...
        function calc(data, cb){
            ...
            if(data.side === 'none'){
                ...
                setTimeout(insert_history(korbit, data), 2000);
            ...
            //포지션수량 < 주문수량
            if(data.openingQty < data.siteMinAmt && data.openingQty >= 0.0001){
        ...
        function order(data, cb){
            ...
            korbit.order('btc_krw', data.side, data.ordPrice, data.ordAmount, function(error, response, body){
                ...
                if(data.idx === 1 && data.ordPrice > 0 && data.ordAmount > 0 && (data.side === 'sell') ){
                    data.start_price = data.ordPrice;
                    data.end_price = data.ordPrice;
                }else if(data.idx > 1 && data.ordPrice > 0 && data.ordAmount > 0 && (data.side === 'sell')){
                    data.end_price = data.ordPrice;
                }
        ...
        function order_info(data, cb){
            ...
            //         data.totalOrdAmt -= unfilled_amount; //주문수량 누적
        ...
        if(data.isContinue === true){
            data.idx += 1;
            setTimeout(divide_exit_korbit(korbit, data, logger), data.ordInterval);//분할주문
        }else{
...
function last_order(korbit, obj){
    return function(){
        async.waterfall([
            ...
            function position(data, cb){
                // korbit.balance(function(error, httpResponse, body){
                //     if(error){
                //         logger.error("코빗 balance 값 조회 error1 : " + error);
                //         data.isError = true;
                //         data.isContinue = true;
                ...
                //     }
                //     if(json.errorCode !== "0"){
                //         logger.error("코빗 balance 값 조회 error2 : " + body);
                //         return cb(null, data);
                //     }
                //     var avail_btc = fixed4(Number(json["btc"].avail));
                    
                //     if(avail_btc >= 0.0001){ //coin_val > data.siteMinVal
                //         data.isContinue = true;
                //     }else{
            ...
            if(data.isContinue === true){
                data.idx += 1;
                setTimeout(divide_exit_korbit(bithumAPI, data, logger), data.ordInterval);//분할주문
            }else{
