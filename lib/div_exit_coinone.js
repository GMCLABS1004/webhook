...
var logger;
module.exports = divide_exit_coinone;
function check_finance_end(){
    return function(){
        //재정거래 끝 인지 확인
        //재정거래 끝O -> 탈출시작
        //재정거래 끝X -> check_finance_end() 호출
    }
}

function divide_exit_coinone(coinone, obj, logObj){
    logger = logObj
      ...
      async.waterfall([
        ...
        function position(data, cb){
            ...
            coinone.balance(function(error, httpResponse, body){
                ...
                if(data.idx === 1){
                    ...
                    data.total_krw =  Math.floor(Number(json["krw"].balance));
                    data.total_btc =  fixed4(Number(json["btc"].balance));
                }
                cb(null, data);
            });
        },
        function calc(data, cb){
            ...
            if(data.side === 'none'){
                ...
                setTimeout(insert_history(coinone, data), 2000);
        ...
        if(data.isContinue === true){
            ...
            // setTimeout(insert_history(coinone, data), 2000);
            // switchOnOff(data, false);
            setTimeout(last_order(coinone, data), 5000);
        }
      });
    }
  }

function last_order(coinone, obj){
    return function(){
        async.waterfall([
            function init(cb){
                var data = {
            ...
            if(data.isContinue === true){
                data.idx += 1;
                setTimeout(divide_exit_coinone(bithumAPI, data, logger), data.ordInterval);//분할주문
            }else{
            ...
            function getUserMargin(cb){ //잔액조회
                coinone.balance(function(error, httpResponse, body){
                    ...
                    if(json.errorCode !== "0"){
                        ...
                        total_krw =  Math.floor(Number(json["krw"].balance));
                        total_btc =  fixed4(Number(json["btc"].balance));
