...
var logger;
module.exports= function check_is_exit(korbit, obj, logObj){
    logger = logObj;
    switchOnOff({apiKey : obj.apiKey,secreteKey : obj.secreteKey}, true);
   
    return function(){
        settings.find({site :'korbit'},function(error, res){
            if(error){
                console.log(error);
                return;
            }
            ...
            if(res[0].isExiting === true){
                ...
                setTimeout(check_is_exit(korbit, obj), 10000);
                return;
            }else if(res[0].isExiting === false){ //분할진입 시작
                logger.info("코인원 진입 시작");
                setTimeout(divide_entry_korbit(korbit, obj),obj.ordInterval);
                return;
            }
        });
    }
...
function divide_entry_korbit(korbit, obj){
    switchOnOff({apiKey : obj.apiKey, secreteKey : obj.secreteKey}, true);
    return function(){
      async.waterfall([
        function init(cb){
            var data = {
                ...
                totalOrdValue : obj.totalOrdValue, //주문넣은 가치 합산(첫 이용가능한 자산 - 현재 이용가능한 자산)
        ...
        function getUserMargin(data, cb){ //잔액조회
            korbit.balances(function(error, response, body){
                ...
                if(data.idx === 1){
                    data.firstMargin =  Math.floor(Number(json["krw"].available) + Number(json["krw"].trade_in_use));
                    data.firstAvailMargin = Math.floor(Number(json["krw"].available));
                    data.total_krw = Math.floor(Number(json["krw"].available) + Number(json["krw"].trade_in_use));
                    data.total_btc = fixed4(Number(json["btc"].available) + Number(json["btc"].trade_in_use));
                }
                
                data.availableMargin =  Math.floor(Number(json["krw"].available));
                data.totalOrdValue = Math.floor(data.firstAvailMargin - data.availableMargin); //주문넣은 가치를 조회 값에 의해 결정
        ...
        function depth(data, cb){
            ...
            korbit.orderbook("btc_krw", function(error, response, body){
                ...
                try{
                    ...
                    logger.error("코빗 매수/매도 값 조회 error2 : " + error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
                ...
                data.buyDepth = {}
                data.sellDepth = {
                    price : Number(json.asks[0][0]),//가격-> $ dollar
                    amount : Number(json.asks[0][1]),//수량-> $ dollar
                    value :  Number(json.asks[0][0]) * Number(json.asks[0][1]) //가치 -> xbt
                }
                cb(null, data);
            });
        },
        function calc(data, cb){
            ...
            if(data.goalValue < (data.totalOrdValue + data.ordValue)){
                ...
                if(diffVal > data.siteMinValue && diffVal <= data.availableMargin ){ //|| diffVal > data.minOrdValue
                    ...
                    //주문X
                    logger.info("가치차이 주문X  : " + diffVal + " " +  data.siteMinValue + " " + diffVal);
                    data.ordPrice = 0; // 주문 가격
                    data.ordAmount = 0;
                    data.ordValue = 0; //주문최소가치로 주문
                    data.isOrdered = false; //주문X
        ...
        function order(data, cb){
            ...
            korbit.order('btc_krw', data.side, data.ordPrice, data.ordAmount, function(error, response, body){
                if(error){
                    logger.info("코빗 주문에러 : "+error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
                ...
                try{
                    ...
                    logger.info("코빗 주문에러 error1 : " + error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
                ...
                logger.info(data.idx+". site : korbit " + "/ side : " + data.side + "/ price : " + price_comma(data.ordPrice) + "/ amount : "+ amount_comma(data.ordAmount) );
                ...
                logger.info("코빗 주문 결과 : "+ body);
                logger.info("코빗 json.orderId : "+ json.orderId);
                // logger.info("코빗 body.orderId : "+ body.orderId);
        ...
        function orderCancel(data, cb){
            ...
            logger.info("orderCancel : "+data.orderID);
            korbit.cancel("btc_krw", data.orderID, function(error, response, body){
                if(error){
                ...
                logger.info("코빗 취소 응답 : "+ body);
                try{
                    var json = JSON.parse(body);
                }catch(error){
                    logger.info("코빗 취소에러 error1 : " + error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
        ...
        function order_info(data,cb){
            ...
            cb(null, data);
            // korbit.order_info("btc_krw", "", data.orderID, function(error, response, body){
            //     if(error){
            //         logger.error("코빗 주문조회 error1 : " + error);
            //         data.isError = true;
            //         data.isContinue = true;
            //         return cb(null, data);
            //     }
            ...
            //         var json = JSON.parse(body);
            //     }catch(error){
            //         logger.info("코빗 주문조회 error2 : " + error);
            //         data.isError = true;
            //         data.isContinue = true;
            //         return cb(null, data);
            //     }
            //     logger.info("코빗 order_info : " + body);

            //     var unfilled_amount =0;
            //     if(data.ordAmount > Number(json[0].filled_amount) ){
            //         console.log("미체결 : " + data.ordAmount - Number(json[0].filled_amount));
            //         unfilled_amount = data.ordAmount - Number(json[0].filled_amount);
            //         data.totalOrdAmount -= unfilled_amount; //주문수량 누적
            //         data.totalOrdValue -= (data.canceledPrice * unfilled_amount);
            //         data.ordAmount = Number(json[0].filled_amount);
            //         data.isContinue = true;
            //     }else{
            //         console.log("완전체결 : " + data.ordAmount - Number(json[0].filled_amount));
            //     }
            //     cb(null, data);
            // });
        },
        ...
        if(data.isContinue === true){ //분할주문 가능
            data.idx += 1;
            setTimeout(divide_entry_korbit(korbit,data), data.ordInterval);//분할주문
            return;
            ...
            // switchOnOff({apiKey : data.apiKey, secreteKey : data.secreteKey},false);
            // insert_history(korbit, data);
            setTimeout(last_order(korbit, data), 5000);
            return;
        }
      });
    }
...
function last_order(korbit, obj){
    return function(){
      async.waterfall([
        function init(cb){
            var data = {
        ...
        function orderCancel(data, cb){
            ...
            korbit.cancel("btc_krw", data.orderID, function(error, response, body){
                if(error){
                    logger.error("코빗 주문취소 error1 : " + error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
                ...
                try{
                    ...
                    logger.error("코빗 주문취소 error2 : " + error);
                    data.isError = true;
                    data.isContinue = true;
                    return cb(null, data);
                }
        ...
        function getUserMargin(data, cb){ //잔액조회
            korbit.balances(function(error, response, body){
                ...
                avail_krw =  Math.floor(Number(json["krw"].available));
                ...
                //     logger.info("avail_krw : " + avail_krw);
                //     logger.info("siteMinValue : " + data.siteMinValue);
                //     data.totalOrdValue -= (data.goalValue - avail_krw);
                //     data.isContinue = true;
                // }
                // else{
                //     logger.info("avail_krw : " + avail_krw);
...
function insert_history(korbit, data){
    ...
    async.waterfall([
        function getUserMargin(cb){ //잔액조회
            ...
            korbit.balances(function(error, response, body){
                ...
                total_krw =  Math.floor(Number(json["krw"].available) + Number(json["krw"].trade_in_use));
                total_btc =  fixed4(Number(json["btc"].available) + Number(json["btc"].trade_in_use));
        ...
        function insertDB(cb){
            ...
            var obj = {
                ...
                price : Math.floor(price),//Math.floor(price),//Math.floor(data.totalOrdValue / data.totalOrdAmount), //가격
                amount : fixed4(amount), //fixed4(data.totalOrdAmount),
                value : Math.floor(value),//Math.floor(value),//Math.floor(data.totalOrdValue), //가치 firstMargin
                feeRate : 0.08 * 0.01,
