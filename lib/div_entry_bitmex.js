        ...
        async.parallel([
                ...
                request(requestOptions, function(err,response,body){
                    ...
                    for(i=0; i<json.length; i++){
                        if(json[i].symbol==='XBTUSD'){
                            data.totalOrdAmount = Math.abs(json[i].currentQty);
                            data.totalOrdValue = Math.abs(json[i].currentQty / json[i].avgEntryPrice);//current_value
            ...
            async.waterfall([
                ...
                function calc(data, cb){
                    ...
                    // //주문목표가치 < 총주문누적 
                    // if(data.goalValue < (data.current_value + data.ordValue)){
                    //     console.log("주문목표가치 < 총주문누적 : " + data.goalValue + " " + data.current_value + " " + data.ordValue);
                    //     //var diffVal =  (data.current_value + data.ordValue) - data.goalValue;
                    //     var diffVal = data.goalValue - data.current_value;
                    //     //가치차이 > 사이트최소가치 && 남은가치 > 최소주문가치
                    //     if(diffVal > data.siteMinValue || diffVal > data.minOrdValue){
                    //         console.log("가치차이만큼 주문1 : " + diffVal );
                    //         //가치차이 만큼 주문
                ...
                function order(data, cb){
                    ...
                    request(requestHeader, function(error, response, body){
                        ...
                        if(json.side === undefined || isNaN(json.orderQty)){ //json.side === undefined || isNaN(json.orderQty)
                            
                            data.isSuccess = false; //주문 실패
                            data.isOrdered = false; //주문취소 안함
                            data.isContinue = true; //재주문계속진행
                            data.ordInterval = data.ordInterval + 5000; //10초뒤 재주문 // 주문실패 누적시 10초씩 누적;
                            if(data.idx === 1){
                ...
                if(data.isContinue === true){ //분할주문 가능
                    ...
                    if(data.isSuccess === true ){ //이전 주문 성공했으면 //|| data.idx ===1
                        data.idx += 1; //인덱스 1 증가
                    }
