        ...
        async.parallel([
            ...
            function depth(cb) { //매수, 매도 목록 조회
                ...
                request(requestHeader, function (error, response, body) {
                        ...
                        else if (json[0]["price"] === undefined || json[1]["price"] === undefined) {
                            data.isError = true;
                            data.isContinue = true;
                        } else {
                            //매수, 매도 목록 저장
            ...
            async.waterfall([
                ...
                function calc(data, cb){
                    ...
                    if(data.side === 'Buy'){ //매수주문 이면
                        ...
                        data.ordAmount = parseInt(Math.floor(ordValue * data.sellDepth.price)/100)*100; //주문수량 = 주문가치 * 매도1가격
                        console.log('수량조정' + data.ordAmount)
                        ...
                        data.ordAmount = parseInt(Math.floor(ordValue * data.buyDepth.price)/100)*100; //주문수량 = 주문가치 * 매도1가격
                    ...
                    console.log('사이트최소 가치 : ' + data.siteMinValue)
                    console.log('사이트최소수량 : ' + data.siteMinAmt)
                    console.log('주문가격 : ' + data.ordPrice)
                    ...
                    if(data.goalValue < data.ordValue){
                        ...
                        data.ordAmount = parseInt(Math.floor(data.goalValue * data.ordPrice)/100)*100; //주문수량
                    ...
                    if(zziggeogi < data.siteMinValue && zziggeogi > 0){ //찌꺼기 포함해서 주문
                        ...
                        data.ordAmount = parseInt(Math.floor(data.ordValue * data.ordPrice)/100)*100; //주문수량
                        data.isOrdered = true; //주문O
                        data.isContinue = false; //다음주문O
                    }
                    ...
                    //남은수량 = (목표가치 - 주문누적가치)*주문가격
                    var remainAmt = (data.goalValue - data.totalOrdValue)*data.ordPrice;

                    //남은가치 < 사이트최소가치, 남은가치 < 최소주문가치
                    if((remainValue < data.siteMinValue && remainValue < data.minOrdValue) || remainAmt < data.siteMinAmt){  //주문X
                    ...
                    //사이트최소가치 < 남은가치 < 최소주문가치 , 남은 주문수량 > 사이트 최소주문수량
                    if(remainValue > data.siteMinValue && remainValue < data.minOrdValue && remainAmt > data.siteMinAmt){
                        console.log("남은가치 > 사이트최소가치");
                        console.log("remainValue : "+ remainValue);
                        console.log("siteMinValue : "+ data.siteMinValue);
                        console.log("minOrdValue : "+ data.minOrdValue);
                        ...
                        //남은가치 만큼 주문
                        data.ordAmount = parseInt(Math.floor(remainValue * data.ordPrice)/100)*100; //주문수량
                        console.log("ordAmount : "+ data.ordAmount);
                        data.ordValue =  data.ordAmount / data.ordPrice; //주문가치 = 주문수량 / 주문가격
                        data.isOrdered = true; //주문O
                        data.isContinue = false; //다음주문X
                    ...
                    if(data.goalValue < (data.totalOrdValue + data.ordValue)){
                        ...
                        if(diffVal > data.siteMinValue || diffVal > data.minOrdValue){
                            ...
                            //가치차이 만큼 주문
                            data.ordAmount = parseInt(Math.floor(diffVal * data.ordPrice)/100)*100; //주문수량 = 가치차이 * 주문가격
                            data.ordValue =  data.ordAmount / data.ordPrice; //주문가치 = 주문수량 / 주문가격
                            data.isOrdered = true; //주문O
                            data.isContinue = false; //다음주문X
                        }
