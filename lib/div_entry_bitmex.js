  ...
  function insert_history(data){
    ...
    async.waterfall([
        ...
        function updatePosition(cb){
            settings.updateOne({site : data.site}, {$set :{side : getType(data.side), side_num : data.side_num , entryPrice : avgPrice, highPrice : avgPrice, lowPrice : avgPrice}}, function(error, res){
                if(error){
                    console.log(error);
                    return;
                }
