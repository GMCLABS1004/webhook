...
module.exports= function check_is_exit(coinone, obj){
      ...
      async.waterfall([
        ...
        function order_info(data,cb){
            ...
            coinone.myOrderInfo(data.orderID, "BTC", function(error, response, body){
                ...
                if(json.errorCode === "0"){
                    if(Number(json.info.remainQty) > 0){
                        data.totalOrdValue -= (data.canceledPrice * Number(json.info.remainQty));
                        console.log("주문가치 누적2 : "+ data.totalOrdValue);
                        data.isContinue=true;
                        return cb(null, data);
                    }
                }
