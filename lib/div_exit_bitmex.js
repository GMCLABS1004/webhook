                ...
                request(requestOptions, function(err,response,body){
                    ...
                    for(i=0; i<json.length; i++){
                        ...
                        if(json[i].symbol==='XBTUSD'){
                            data.totalOrdAmt = data.goalAmt - data.openingQty;
                            data.totalOrdValue = Math.abs(data.totalOrdAmt / json[i].avgEntryPrice);//current_value
...
function insert_history(data){
    ...
    async.waterfall([
        ...
        //         var json = JSON.parse(body);
        //         for(i=0; i<json.length; i++){
        //             if( (json[i].currentQty > 0 || json[i].currentQty <0)  && json[i].symbol==='XBTUSD'){
        //                 console.log("탈출 실패");
        //                 isOrdered = false;
        //             }
        //         }
        ...
        function insert_benefit_history(cb){
            ...
            benefitDB.insertMany(obj, function(error, res){
                ...
                setTimeout(check_benefit_his(), seconds * 1000);
                cb(null);
            });
        },
