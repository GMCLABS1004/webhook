            ...
            settings.findOne({site : data.site}, function(error, json){
                ...
                if(data.type_log === 'trailingStop'){ //트레일링 스탑 -> 상태변화X
                    settings.findByIdAndUpdate(
                        json._id,
                        {$set : {entryPrice : json.entryPrice, highPrice : json.entryPrice, lowPrice : json.entryPrice}}, //
                        function(error, res){
                            if(error){
                                console.log(error);
                                return cb(null);
                    ...
                    settings.findByIdAndUpdate(json._id, {$set :{side : 'exit', side_num : data.side_num, entryPrice : json.entryPrice, highPrice : json.entryPrice, lowPrice : json.entryPrice}}, function(error, res){
                        if(error){
                            console.log(error);
                            return;
                        }
