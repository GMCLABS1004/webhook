var numeral = require('numeral');
var mongoose = require('mongoose');
var randomFloat = require('random-float');
var randomInt = require('random-int');
var crypto = require("crypto");
var request = require("request");
var async = require('async');
const winston = require('winston');
require('winston-daily-rotate-file');
require('date-utils');

var settings = require("../models/setting");
var webSetting = require('../webSetting.json');
var orderDB = require('../models/order');
var position2 = require('../models/position2');
var benefitDB = require('../models/benefit');
// var logger;
// var logfileName1 = '../log/marginTrade' +'.log'; //로그파일 경로1
// var logfileName2 = '../log/marginTrade' +'.debug.log'; //로그파일 경로2
//create_logger(logfileName1, logfileName2, function(loggerHandle){ logger = loggerHandle}); //logger 생성
mongoose.connect(webSetting.dbPath, function(error){
    if(error){
      console.log(error);
      return;
    }
});  


// var data = {
//     idx : 1,
//     url : 'https://testnet.bitmex.com',
//     apiKey : '-2YJMJOGLRMvUgaBD1_KzbLt',
//     secreteKey : 'aEvaHawjJK5bU3ePZqNtzSt7I6smHfelkDRV6YS_lmmQffwd',
//     symbol : 'XBTUSD',
//     goalAmt : 0,
//     totalOrdAmt : 0,
//     openingQty : 0, //진입한 포지션 수량 
//     side : "",
//     minAmtRate : 0.07, //최소주문비율
//     maxAmtRate : 0.09, //최대주문비율
//     isOrdered : false, //주문시도 여부
//     isSuccess : false, //주문성공 여부
//     isContinue : false, //주문분할 계속할지 여부
// }

// setTimeout(divide_exit_bitmex(data), 0);
var logger;
module.exports = function divide_exit_bitmex(obj, logObj){
    logger = logObj;
    switchOnOff(obj,true);
    return function(){
        var data = {
            site : obj.site,
            idx : obj.idx,
            url : obj.url,
            apiKey : obj.apiKey,
            secreteKey : obj.secreteKey,
            symbol : obj.symbol,
            ordInterval : obj.ordInterval,
            minOrdAmt : obj.minOrdAmt,
            goalAmt : obj.goalAmt,
            totalOrdAmt : obj.totalOrdAmt,
            totalOrdValue : obj.totalOrdValue,
            openingQty : 0, //진입한 포지션 수량 
            side : "",
            side_num : obj.side_num,//스클립트 번호
            start_price : obj.start_price,//시작가격
            end_price : obj.end_price, //종료가격
            minAmtRate : obj.minAmtRate, //최소수량비율
            maxAmtRate : obj.maxAmtRate, //최대수량비율
            orderID : obj.orderID, //주문id
            start_time : obj.start_time, //시작시간
            type_log : obj.type_log, //주문종류
            start_asset_sum : obj.start_asset_sum,  //첫 자산들 총합
            end_asset_sum : obj.end_asset_sum, //탈출전 자산 합
            isOrdered : false, //주문시도 여부
            isSuccess : false, //주문성공 여부
            isContinue : false, //주문분할 계속할지 여부
            isError : false //에러여부
        }
        async.parallel([
            function end_asset_sum(cb){ //탈출전 자산합 조회
                if(data.idx !== 1){
                    return cb(null);
                }

                //최초 한번만 실행
                get_total_asset("desc",function(error, asset){
                    if(error){
                        console.log(error);
                        return;
                    }
                    data.end_asset_sum = asset; //탈출전 자산 총합 저장
                    cb(null);
                });
            },
            function start_asset_sum(cb){ //모든 계정들 각각의 첫자산을 모두 총합
                if(data.idx !== 1){
                    return cb(null);
                }

                //최초 한번만 실행
                get_total_asset("asc",function(error, asset){
                    if(error){
                        console.log(error);
                        return;
                    }
                    data.start_asset_sum = asset; //첫자산 총합 저장
                    cb(null);
                });
            },
            function position(cb){
                var requestOptions = setRequestHeader(data.url, data.apiKey, data.secreteKey, 'GET','position','');//'currency=XBt'
                request(requestOptions, function(err,response,body){
                    if(err){
                        console.log(err);
                        data.isError = true;
                        data.isContinue = true;
                        return cb(null, data);
                    }
                    //console.log(body);
                    var json = JSON.parse(body);
                    
                    for(i=0; i<json.length; i++){
			logger.info("json[i].currentQty : "+ json[i].currentQty);
			logger.info("json[i].symbol : "+json[i].symbol);
                        if(json[i].currentQty > 0 && json[i].symbol==='XBTUSD'){
                            //console.log("매수");
                            data.openingQty = json[i].currentQty; //진입해 있는 수량 저장
                            data.side = "Sell"; //현재포지션은 Buy, 주문은 Sell로
                        }else if(json[i].currentQty < 0 && json[i].symbol==='XBTUSD'){
                            //console.log("매도");
                            data.openingQty = Math.abs(json[i].currentQty); //진입해 있는 수량 저장
                            data.side = "Buy"; //현재포지션은 Sell, 주문은 Buy로
                            //계산값 DB저장용
                        }else if(json[i].currentQty === 0 && json[i].symbol==='XBTUSD'){
                            //console.log("매도");
                            data.openingQty = 0; //진입해 있는 수량 0개
                            data.side = "NONE"; //현재포지션 NONE, 주문 X
                            //계산값 DB저장용
                        }else{
                            continue;
                            // data.openingQty = 0; //진입해 있는 수량 0개
                            // data.side = "NONE"; //현재포지션 NONE, 주문 X
                        }
                        //탈출은 진입해있는 수량을 0으로 만드는것이 목적
                        //분할횟수 첫번째면
                        if(data.idx === 1 && data.goalAmt === 0){
                            data.goalAmt = data.openingQty; //목표수량 = 진입해있는 수량, 로직 실행시 오직 한번만 저장
                        }

                        if(json[i].symbol==='XBTUSD'){
                            data.totalOrdAmt = data.goalAmt - data.openingQty; //실제 주문한 수량 = 목표수량 - 진입 수량
                            data.totalOrdValue = Math.abs(data.totalOrdAmt / json[i].avgEntryPrice);//실제 주문한 가치 =  실제 주문한 수량 / 평단가
                            data.avgEntryPrice = json[i].avgEntryPrice
                            if(isNaN(data.totalOrdValue)){
                                data.totalOrdValue=0;
                            }
                        }
                    }

		    logger.info("test");
                    logger.info("data.site : "+ data.site);
                    logger.info("data.openingQty : "+ data.openingQty);
                    logger.info("data.side : "+ data.side);
                    logger.info("data.goalAmt : "+ data.goalAmt);
                    logger.info("data.avgEntryPrice : "+ data.avgEntryPrice);

                    logger.info("data.totalOrdAmt : "+ data.totalOrdAmt);
                    logger.info("data.totalOrdValue : "+ data.totalOrdValue);
                    cb(null);
                });
            },
            function depth(cb){ //매수, 매도 목록 조회
                var requestHeader = setRequestHeader(data.url, data.apiKey, data.secreteKey, 'GET', 'orderbook/L2', 'symbol='+data.symbol+'&depth=1');
                request(requestHeader, function(error, response, body){
                    if(error){
                        console.log(error);
                        data.isError = true;
                        data.isContinue = true;
                        return cb(null, data);
                    }
                    var json = JSON.parse(body);
                    console.log("----------------"+json[0])
                    if(json[0]["price"] === undefined || json[1]["price"] === undefined){
                        data.isError = true;
                        data.isContinue = true;
                    }else{ //조회성공
                        //매수, 매도 목록 저장
                        data.sellDepth = {
                            price : json[0].price,//가격-> $ dollar
                            amount : json[0].size,//수량-> $ dollar
                            value :  json[0].size / json[0].price //가치 -> xbt
                        }
                        data.buyDepth = {
                            price : json[1].price,//가격-> $ dollar
                            amount : json[1].size, //수량-> $ dollar
                            value : json[1].size / json[1].price  //가치 -> xbt
                        }
                    }

                    cb(null);
                });
            },
        ], function(error, results){
            if(error){
                console.log(error);
                return;
            }

            async.waterfall([
                function init(cb){
                    cb(null, data);
                },
                function calc(data, cb){
                    if(data.isError === true){
                        return cb(null, data);
                    }

                    var rate = (randomFloat(data.minAmtRate, data.maxAmtRate));
                    var ordAmount=0;
                    var remainValue=0;
                    if(data.side === 'NONE'){ //현재 포지션 === NONE
                        logger.info("NONE");
                        data.isOrdered = false; //주문X
                        data.isContinue = false; //다음주문X
                        return cb(null, data);
                    }
                    else if(data.side === 'Buy'){ //현재 포지션 === Buy
                        logger.info("Buy");

                        data.ordAmount = parseInt(Math.floor(data.sellDepth.amount * rate)/100)*100; //주문수량 = 매도1수량 * 비율
                        data.ordPrice = data.sellDepth.price; //주문가격 = 매도1 가격
                        data.ordValue = data.ordAmount / data.ordPrice; //주문 가치 = 주문수량 / 주문가격

                        data.isOrdered = true; //주문O
                        data.isContinue = true; //다음주문O

                    }else if(data.side === 'Sell'){
                        logger.info("Sell");
                        data.ordAmount = parseInt(Math.floor(data.buyDepth.amount * rate)/100)*100
                        data.ordPrice = data.buyDepth.price; //주문가격 = 매수1가격
                        data.ordValue = data.ordAmount / data.ordPrice; //주문 가치 = 주문수량 / 주문가격
                        data.isOrdered = true; //주문O
                        data.isContinue = true; //다음주문O
                    }

                    // 주문수량 < 최소주문수량
                    if(data.ordAmount < data.minOrdAmt ){
                        data.ordAmount = data.minOrdAmt; //주문수량 = 최소주문수량
                        data.isOrdered = true; //주문O
                        data.isContinue = true; //다음주문O
                    }

                    //진입해있는 수량 < 주문수량
                    if(data.openingQty < data.ordAmount){
                        logger.info("포지션수량 < 주문수량");
                        data.ordAmount = data.openingQty; //주문수량 = 진입해있는 수량
                        data.isOrdered = true; //주문O
                        data.isContinue = false; //다음주문X
                    }

                    //진입해있는 < 주문수량
                    if(data.openingQty < data.siteMinAmt){
                        logger.info("포지션수량 < 사이트최소수량");
                        data.ordAmount = 0; //주문수량 = 0
                        data.isOrdered = false; //주문O
                        data.isContinue = false; //다음주문X
                    }

                    cb(null, data); //주문X
                },
                function order(data, cb){
                    if(data.isError === true){
                        return cb(null, data); //주문X, 로직종료
                    }

                    if(data.isOrdered === false){
                        return cb(null, data); //주문X, 로직종료
                    }

                    //탈출 주문
                    var requestHeader = setRequestHeader(data.url, data.apiKey, data.secreteKey, 'POST','order',
                                            {symbol : data.symbol, side : data.side, price : data.ordPrice, orderQty : data.ordAmount, ordType : "Limit", text : "auto"});
                    request(requestHeader, function(error, response, body){
                        if(error){
                            console.log(error)
                            data.isError = true;
                            data.isContinue = true;
                            return cb(null, data);
                        }

                        //console.log("주문1 : " + body);
                        var json = JSON.parse(body);

                        logger.info(data.idx+". site : "+ data.site + "/ side : " + json.side + "/ price : " + price_comma(json.price) + "/ amount : "+ amount_comma(json.orderQty) );
                        if(json.side === undefined || isNaN(json.orderQty)){ //주문실패
                            logger.info("주문실패");
                            //logger.info(body);
                            data.isSuccess = false; //주문 실패
                            data.isOrdered = false; //주문취소 안함
                            data.isContinue = true; //분할주문 계속 진행(재주문)
                            data.ordInterval = data.ordInterval + 2000; //2초뒤 재주문 // 주문실패 누적시 2초씩 누적;
                            if(data.idx === 1){ //첫주문인경우
                                data.start_price = data.ordPrice; //시작가격 = 주문가격
                                data.end_price = data.ordPrice; //마지막가격 = 주문가격
                            }else if(data.idx > 1){ //첫주문이 아닌경우
                                data.end_price = data.ordPrice; //마지막가격 = 주문가격
                            }
                            console.log("주문실패 -> " + (data.ordInterval / 1000) +"초 뒤 재주문");

                            return cb(null, data);
                        }

                        //주문성공했고, 첫주문이면
                        if(data.idx === 1 && data.ordPrice > 0 && data.ordAmount > 0 && (data.side === 'Buy' || data.side=='Sell') ){
                            data.start_price = data.ordPrice; //시작가격 = 주문가격
                            data.end_price = data.ordPrice; //종료가격 = 주문가격
                        }//주문성공했고, 첫주문이 아니면
                        else if(data.idx > 1 && data.ordPrice > 0 && data.ordAmount > 0 && (data.side === 'Buy' || data.side=='Sell')){
                            data.end_price = data.ordPrice; //종료가격 = 주문가격
                        }

                        data.orderID = json.orderID; //취소위해 주문ID저장
                        data.totalOrdAmt += data.ordAmount; //주문누적수량 = 주문누적수량 + 주문수량
                        data.totalOrdValue += (data.ordAmount / data.ordPrice); //주문누적가치 = 주문누적가치 + (주문수량 / 주문가격)
                        if(data.ordInterval >= 10000){ //실패 이력 있으면
                            data.ordInterval = 1000; //다음 주문은 1초뒤 주문
                        }

                        data.isSuccess = true; //주문성공
                        cb(null, data);
                    });
                },
                function orderCancel(data, cb){
                    if(data.isError === true){
                        return cb(null, data);
                    }

                    if(data.isOrdered === false){
                        return cb(null, data); //주문X
                    }

                    //주문취소
                    var requestHeader = setRequestHeader(data.url, data.apiKey, data.secreteKey, 'DELETE','order',
                    {orderID : data.orderID});
                    request(requestHeader, function(error, response, body){
                        if(error){
                            console.log(error)
                            //res.send(error);
                            data.isError = true;
                            data.isContinue = true;
                            return cb(null, data);
                        }
                        var json = JSON.parse(body);

                        if(typeof(json.error) === 'object' ){
                            return cb(null, data);
                        }

                        var remainAmt = json[0].orderQty - json[0].cumQty; //남은수량 = 주문수량 - 실제 체결된 수량

                        if(remainAmt > 0){ //남은수량이 있으면
                            data.totalOrdAmt -= remainAmt; //주문누적수량 = 주문누적수량 - 남은수량
                            data.totalOrdValue -= (remainAmt/data.ordPrice); //주문누적가치 = 주문누적가치 - 남은가치
                            data.isContinue = true; //분할주문계속
                        }

                        cb(null, data);
                    });
                }
              ],function(error, data){
                if(error){
                    console.log(error);
                    logger.info("분할주문종료");
                    //switchOnOff(data, false);
                    insert_history(data);
                    return;
                }
                // logger.info("idx : " +data.idx);
                // // logger.info("goalAmt : " +data.goalAmt);
                // // logger.info("totalOrdAmt : " +data.totalOrdAmt);
                // logger.info("goalAmt : " +data.goalAmt);
                // logger.info("totalOrdAmt : " +data.totalOrdAmt);
                // logger.info("ordAmount : " +data.ordAmount);
                // logger.info("isOrdered : " +data.isOrdered);
                // logger.info("isSuccess : " +data.isSuccess);
                // logger.info("isContinue : " +data.isContinue);
                logger.info();
                if(data.isContinue === true){ //분할주문 계속

                    if(data.isSuccess === true){ //이전 주문 성공했으면
                        data.idx += 1; //분할횟수 1 증가
                    }

                    setTimeout(divide_exit_bitmex(data, logger), data.ordInterval);//분할주문시작
                    return;
                }else{ //분할주문 X
                    logger.info("분할주문종료");
                    //switchOnOff(data, false);
                    insert_history(data); //
                    return;
                }
              });
        });
    }
  }

function switchOnOff(data, isOnOff){
    //탈출중 플래그 ON
    //수정된 원화가치 저장
    // settings_v2.updateOne({currency : order_currency + "_" + payment_currency}, {$set : {changed_krw : changed_krw}},function(err, json){
    //     if(err){
    //         console.log(err);
    //         return;
    //     }
    //     console.log("changed_krw 업데이트");
    // });
    settings.updateOne({
        site : data.site
    },{
        $set : {
            isExiting : isOnOff
        }
    },function(error, res){
        if(error){
            logger.info(error);
            return;
        }
        console.log(res);
    });
}


function get_total_asset(isSort, callback){
    var list = [];
    var total_asset=0;
    for(var i=1; i<=10; i++){
      orderDB.findOne({site : "bitmex"+i}).sort({start_time : isSort}).limit(1).exec(function(error, json){
        if(error){
          console.log(error);
          return;
        }

        list.push(json);
        if(json!== null){
          total_asset += json.totalAsset;
        }

        if(list.length === 10){
          callback(null, total_asset);
        }
      });
    }
  }


function setRequestHeader(url, apiKey, apiSecret, verb, endpoint, data){
    path = '/api/v1/'+ endpoint;
    expires = new Date().getTime() + (60 * 1000); // 1 min in the future
    var requestOptions;
    if(verb === 'POST' || verb === 'PUT' || verb === 'DELETE'){
        var postBody = JSON.stringify(data);
        var signature = crypto.createHmac('sha256', apiSecret).update(verb + path + expires + postBody).digest('hex');
        var headers = {
            'content-type' : 'application/json',
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
            'api-expires': expires,
            'api-key': apiKey,
            'api-signature': signature
        };
        requestOptions = {
            headers: headers,
            url: url+path,
            method: verb,
            body: postBody
        };
    }else{ //'GET'
        var query = '?'+ data;
        var signature = crypto.createHmac('sha256', apiSecret).update(verb + path + query + expires).digest('hex');
        var headers = {
          'content-type' : 'application/json',
          'Accept': 'application/json',
          'X-Requested-With': 'XMLHttpRequest',
          'api-expires': expires,
          'api-key': apiKey,
          'api-signature': signature
        };
        requestOptions = {
            headers: headers,
            url: url+path + query,
            method: verb
        };
    }
    return requestOptions;
}


function insert_history(data){
    var start_time = data.start_time;
    var end_time = new Date();
    end_time = end_time.getTime() + (1000 * 60 * 60 * 9);

    var walletBalance = 0;
    var benefit =0;
    var benefitRate = 0;
    var isOrdered = true
    var avgPrice =0;
    var benefitObj = {}
    async.waterfall([
        function margin(cb){ //탈출후 자산조회
            var requestOptions = setRequestHeader(data.url, data.apiKey, data.secreteKey, 'GET','user/margin','currency=XBt');
            request(requestOptions, function(error, response, body){
                if(error){
                    console.log(error);
                    return;
                }

                var json = JSON.parse(body);
                walletBalance = json.walletBalance / 100000000; //탈출후 자산조회

                //탈출 플래그 OFF로 변경 -> 진입가능으로 상태값 변경
                switchOnOff(data,false);

                cb(null);
            });
        },
        function getOrderHistory(cb){ //수익율 계산
            orderDB.find({site : data.site}).sort({end_time : "desc"}).exec(function(error, res){
                if(error){
                    console.log(error);
                    return;
                }
                if(res.length > 0){
                    benefit= walletBalance - res[0].totalAsset; //수익 = 탈출후자산 - 진입자산
                    benefitRate = (benefit / res[0].totalAsset) * 100; //수익율 = (수익 / 진입자산) * 100
                }else{
                    benefit =0;
                    benefitRate =0;
                }
                cb(null);
            });
        },
        function insert_order_history(cb){ //거래내역 저장
            var obj={
                site : data.site, //계정 ex) bitmex1 or bitmex5
                symbol : 'XBTUSD',
                totalAsset : 0, //총자산
                //type : (isOrdered === true)? "exit" : "exit 실패", //탈출
                type : "exit", //탈출
                side : data.side, //Buy or Sell
                side_num : data.side_num, //스크립트 번호
                totalAsset : walletBalance, //총자산
                start_price :  fixed1(data.start_price), //시작가격
                end_price :  fixed1(data.end_price), //마지막가격
                price :  fixed1(data.totalOrdAmt / data.totalOrdValue), //평단가
                amount : data.totalOrdAmt, //주문누적수량
                value : fixed4(data.totalOrdValue), //주문누적가치
                benefit : fixed8(benefit), //수익
                benefitRate : fixed4(benefitRate), //수익율
                start_time : data.start_time, //시작시간
                end_time : end_time, //종료시간
                div_cnt : data.idx, //분할횟수
                type_log : data.type_log, //주문종류 (트레일링스탑 or 트레이딩뷰 분할주문 등등)
                isSend : false //telegram 전송여부
            }
            avgPrice = obj.price;
            console.log("type_log1 : "+ data.type_log);

            //거래내역 저장
            orderDB.insertMany(obj, function(error, res){
                if(error){
                    console.log(error);
                    return;
                }
                console.log(res);
                cb(null);
            });
        },
        function insert_benefit_history(cb){ //수익내역 저장
            var obj = {
                site : data.site,
                start_asset_sum : fixed8(data.start_asset_sum), //시작 자산들 총합(탈출전)
                end_asset_sum : 0, //최근 자산들 총합(탈출전)
                before_asset_sum : 0,  //최근 자산들 총합(탈출전)
                after_asset_sum : 0,//최근 자산들 총합(탈출후)
                benefit : fixed8(benefit), //이번거래로 생긴 수익
                benefitRate : 0, //탈출전 총자산 대비 수익율
                type_log : data.type_log, //trailingStop, rentry, manual etc..
                timestamp : end_time,//탈출 시간
                start_time : start_time, //시작시간
                end_time : end_time, //종료시간
            }

            console.log("type_log2 : "+ data.type_log);
            //수익내역 저장
            benefitDB.insertMany(obj, function(error, res){
                if(error){
                    console.log(error);
                    return;
                }
                var seconds= randomInt(1, 10);
                setTimeout(check_benefit_his(), seconds * 1000); //수익내역 저장시 충돌을 막기 위해 랜덤초 만큼 뒤에 DB에저장
                cb(null);
            });
        },
        function updatePosition(cb){

            settings.findOne({site : data.site}, function(error, json){
                if(error){
                    console.log(error);
                    return;
                }

                if(data.type_log === 'trailingStop'){  //주문종류가 트레일링스탑인 경우 -> 진입가, 고가, 저가 업데이트
                    settings.findByIdAndUpdate(
                        json._id,
                        {$set : {entryPrice : avgPrice, highPrice : avgPrice, lowPrice : avgPrice}}, //
                        function(error, res){
                            if(error){
                                console.log(error);
                                return cb(null);
                            }
                            return cb(null);
                        }
                    )
                }else{ //주문종류가 트레일링스탑이 아닌경우 -> 프로그램포지션 exit, 스크립트번호, 진입가, 고가, 저가 업데이트
                    settings.findByIdAndUpdate(json._id, {$set :{side : 'exit', side_num : data.side_num, entryPrice : avgPrice, highPrice : avgPrice, lowPrice : avgPrice}}, function(error, res){
                        if(error){
                            console.log(error);
                            return;
                        }
                        console.log(res);
                        return cb(null);
                    });
                }
            });
        },

    ], function(error, results){
        if(error){
            console.log(error);
            return;
        }
    })
}

//수익내역 저장
function check_benefit_his(){
    return function(){
        filled_data = {};
        var end_asset_sum = 0;
        var before_asset_sum = 0;
        //var after_asset_sum = 0;
        async.waterfall([
            function find_filled_benefit(cb){
                console.log("find_filled_benefit");
                //체결된 주문중 가장 최근 주문 1개
                benefitDB.find({end_asset_sum : {$gt : 0}}).sort({"start_time" : "desc"}).limit(1).exec(function(error, json){
                    if(error){
                        console.log(error);
                        return;
                    }
                    console.log(json);
                    if(json.length > 0){
                        filled_data=new Object(json[0]);
                        end_asset_sum = filled_data.after_asset_sum;
                        before_asset_sum = filled_data.after_asset_sum;
                        cb(null);
                    }else{
                        return;
                    }

                });
            },
            function find_uncecked_benefit(cb){
                console.log("find_uncecked_benefit");
                // end_asset_sum : 0, //최근 자산들 총합(탈출전)
                // before_asset_sum : 0,  //최근 자산들 총합(탈출전)
                // after_asset_sum : 0,//최근 자산들 총합(탈출후)
                //기록안된 주문 수익율 계산
                benefitDB.find({end_asset_sum : 0, before_asset_sum : 0, after_asset_sum : 0}).sort({"start_time" : "asc"}).exec(function(error, json){
                    if(error){
                        console.log(error);
                        return;
                    }

                    for(var i=0; i<json.length; i++){
                        var obj  = {
                            end_asset_sum : fixed8(end_asset_sum),
                            before_asset_sum : fixed8(before_asset_sum),
                            after_asset_sum : fixed8(before_asset_sum + json[i].benefit),
                            benefitRate : (json[i].benefit / end_asset_sum) * 100,
                        }
                        console.log(obj);
                        benefitDB.findByIdAndUpdate(
                            json[i]._id,
                            {$set : obj},
                            function(error, res){
                                if(error){
                                    console.log(error);
                                    return;
                                }
                            }
                        )
                        end_asset_sum = obj.after_asset_sum;
                        before_asset_sum = obj.after_asset_sum;
                    }
                    cb(null);
                });
            }
        ], function(error, res){
            if(error){
                console.log(error);
                return;
            }
            console.log(res);
        });
    }
}



function isPosition(size){
    if(size > 0){
        return "long"
    }else if(size < 0){
        return "short";
    }else{
        return "exit";
    }
  }


function fixed8(num){
    var str = new String(num);
    var arr = str.split(".");
	if(arr.length>1){
        var str2 = arr[1].slice(0,8);
    	return Number(arr[0] + '.' + str2);
	}
	return Number(arr[0])
}

function fixed4(num){
    var str = new String(num);
    var arr = str.split(".");
	if(arr.length>1){
		    var str2 = arr[1].slice(0,4);
    	return Number(arr[0] + '.' + str2);
	}
	return Number(arr[0])
}

function fixed1(num){
    var str = new String(num);
    var arr = str.split(".");
    if(arr.length>1){
        var str2 = arr[1].slice(0,1);
        return Number(arr[0] + '.' + str2);
    }
    return Number(arr[0]);
}
function fixed2(num){
    var str = new String(num);
    var arr = str.split(".");
    if(arr.length>1){
        var str2 = arr[1].slice(0,2);
        return Number(arr[0] + '.' + str2);
    }
    return Number(arr[0]);
}

function price_comma(num){
    var price = Number(num)
    if(price < 100){ //가격이 100원보다 작으면 ',' 표시 안하고 그대로 출력
        return price;
    }else{ //가격이 100원보다 크면 ',' 표시
        return numeral(price).format( '₩0,0' )
    }
  }

function amount_comma(num){
    var coin = Number(num);
    if(coin >= 0.000001){
        return numeral(coin).format( '₩0,0.0000' ); // 1000.00000123 =>  1,000.00000123
    }else{
        return coin.toFixed(4); // 0.00000078 -> 0.00000078
    }
  }


/**
 *
 * @param {String} info 레벨 로그 logfileName1
 * @param {String} debug 레벨 로그 logfileName2
 */
function create_logger(logfileName1, logfileName2, callback){
    var handle =  winston.createLogger({
        level: 'debug', // 최소 레벨
        // 파일저장
        transports: [
            new winston.transports.DailyRotateFile({
                level : 'info',
                filename : logfileName1, // log 폴더에 system.log 이름으로 저장
                zippedArchive: false, // 압축여부
                maxFiles: '14d',
                format: winston.format.printf(
                    info => `${new Date().toFormat('YYYY-MM-DD HH24:MI:SS')} [${info.level.toUpperCase()}] - ${info.message}`)
            }),

            new winston.transports.DailyRotateFile({
                filename : logfileName2, // log 폴더에 system.log 이름으로 저장
                zippedArchive: false, // 압축여부
                maxFiles: '14d',
                format: winston.format.printf(
                    info => `${new Date().toFormat('YYYY-MM-DD HH24:MI:SS')} [${info.level.toUpperCase()}] - ${info.message}`)
            }),
            // 콘솔 출력
            new winston.transports.Console({
                format: winston.format.printf(
                    info => `${new Date().toFormat('YYYY-MM-DD HH24:MI:SS')} [${info.level.toUpperCase()}] - ${info.message}`)
            })
        ]
    });

    callback(handle);
  }
