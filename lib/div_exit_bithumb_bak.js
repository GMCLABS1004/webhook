...
var logger;
module.exports = function divide_exit_bithumb(bithumAPI, obj, logObj){
    logger = logObj
        ...
        function order_info(data, cb){
            ...
            bithumAPI.bithumPostAPICall('/info/orders', rgParams, function(error, response, body){
                ...
                if(json.status === "0000"){
                    ...
                    data.totalOrdValue += (Number(json.data[0].units_remaining) * data.ordPrice);
                }
...
function insert_history(data){
    ...
    async.waterfall([
        function getOrderHistory(cb){
            orderDB.find({type : "entry"}).sort({end_time : "desc"}).exec(function(error, res){
                if(error){
                    console.log(error);
                    return;
                }
                if(res[0].side === 'ask'){
                    benefit= Math.floor(data.totalOrdValue) - res[0].value;
                    benefitRate = (benefit / res[0].totalAsset);
                }
                cb(null);
            });
        },
        function position(cb){
            ...
            bithumAPI.bithumPostAPICall('/info/balance', rgParams, function(error, response, body){
                ...
                try{
                    ...
                    console.log("빗썸 balance 값 조회 error2 : " + error);
                    return;
                }

                if(json.status !== "0000"){
                    console.log("빗썸 balance 값 조회 error3 : " + error);
                    return;
                }

                walletBalance = Number(json.data["total_krw"]);
                cb(null);
            });
        },
